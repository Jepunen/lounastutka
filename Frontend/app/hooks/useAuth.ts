...
import { browserSupportsWebAuthn, WebAuthnAbortService, WebAuthnError } from "@simplewebauthn/browser";
  ...
  const loginWithWebauth = useCallback(
    async (email: string) => {
      ...
      try {
        ...
        // If the user reloads page during the Webauthn call, we need to abort the previous
        // src: https://simplewebauthn.dev/docs/packages/browser#troubleshooting
  ...
  const registerWithWebauth = useCallback(
    async (email: string) => {
      ...
      try {
        ...
        // If the user reloads page during the Webauthn call, we need to abort the call 
        // src: https://simplewebauthn.dev/docs/packages/browser#troubleshooting
        if (err instanceof WebAuthnError && err.code === "ERROR_CEREMONY_ABORTED") {
          WebAuthnAbortService.cancelCeremony();
        } else {
